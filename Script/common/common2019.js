    ...
    MediaPlayer: function (url, closeCallback) {
        ...
        if ($.mobileOS.any() || $.isTablet()) {
            switch (mvInfo.find('.mvType').text()) {
                case '01': window.open("http://m.youtube.com/watch?v=" + mvInfo.find('.mvInfoID').text(), "movie"); break;
            	case '02': window.open("http://tvcast.naver.com/v/" + mvInfo.find('.tvcastID').text(), "movie"); break;
            	case '03': window.open("https://www.vlive.tv/embed/" + mvInfo.find('.mvInfoID').text(), "movie"); break;
            }
